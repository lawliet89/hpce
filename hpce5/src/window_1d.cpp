               ...
               const uint8_t bit_width)
{
  ...
  if (!serial_hack) {
    ...
    uint64_t chunks_per_img = ((img_height * img_w_bytes) + chunk_size - 1) / chunk_size;
    uint64_t chunks_per_img_padded = (((img_height+n_levels) * img_w_bytes) + chunk_size - 1) / chunk_size;
    ...
    for (int m = 0; m < num_windows_assigned; ++m) {
      ...
      if (i == img_w_bytes - 1) {
        for (uint32_t ii = 1; ii <= n_depth; ++ii) {
          ...
          acc1 += (acc1<in_buf ? buf_size : 0);
          uint8_t *acc2 = curr_chunk + j +ii - (2*n_levels - n_depth)*img_w_bytes - n_depth;
          acc2 += (acc2<in_buf ? buf_size : 0);
